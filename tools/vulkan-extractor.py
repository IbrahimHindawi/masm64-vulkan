        ...
        for field in cursor.get_children():
            if field.kind == CursorKind.FIELD_DECL:
                ...
                if field.type.kind == TypeKind.POINTER:
                    ...
                    if field_type != None:
                        field_type += ' ?' 
                    elif typedefdict.get(field.type.spelling, False) == True:
                        field_type = field.type.spelling 
                        field_type += ' ?' 
                    else:
                        if field.type.spelling in ('VkStructureType', 'VkAccessFlags', 'VkImageCreateFlags', 'VkFormatFeatureFlags'):
                            field_type = 'dword'
                            field_type += ' ?' 
                        else:
                            field_type = field.type.spelling
    ...
    if cursor.kind == CursorKind.TYPEDEF_DECL:
        ...
        if cursor.underlying_typedef_type.kind == TypeKind.ELABORATED:
            ...
            if field_type != None:
                ...
                typedefdict[cursor.spelling] = True
...
def visit(cursor):
    ...
    if cursor.location.file and not cursor.location.file.name.endswith(filename):
        return
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python vulkan-extractor.py <path_to_vulkan_core.h> <filename>")
        return
